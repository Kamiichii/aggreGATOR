...
export async function deleteFeedFollow(user, url) {
    const thisFeed = await getFeed(url);
    await db
        .delete(feedFollows)
        .where(and(eq(feedFollows.userId, user.id), eq(feedFollows.feedId, thisFeed.id)));
}
